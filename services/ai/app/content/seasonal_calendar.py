...
def _last_friday_of_november(year: int) -> date:
    """Return the last Friday of November, the Egyptian White Friday."""
    november_last = date(year, 11, 30)
    offset = (november_last.weekday() - 4) % 7
    return november_last - timedelta(days=offset)
...
_STATIC_WINDOWS: tuple[_SeasonalWindow, ...] = (
    ...
    _SeasonalWindow("mothers_day", "Mother's Day", (3, 21), (3, 21)),
    ...
    _SeasonalWindow("egyptian_valentines_day", "Egyptian Valentine's Day", (11, 4), (11, 4)),
    _SeasonalWindow("singles_day", "Singles' Day", (11, 11), (11, 11)),
)

_COMPUTED_WINDOWS: tuple[_ComputedWindow, ...] = (
    _ComputedWindow("white_friday", "White Friday", _last_friday_of_november),
)
...
def observances_for_week(week_start: date) -> list[dict[str, str]]:
    ...
    return [
        ...
        if window.observed
        and any(
            _in_window(_month_day(day), window.start, window.end) for day in week_days
        )
    ] + [
        {"id": window.id, "label": window.label}
        for window in _COMPUTED_WINDOWS
        if window.observed and any(window.date_for(day.year) == day for day in week_days)
    ]