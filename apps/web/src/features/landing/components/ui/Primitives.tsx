...
export function Chip({
  ...
  children: ReactNode
  tone?: 'neutral' | 'competitor' | 'signal'
  className?: string
}) {
  const tones = {
  ...
  return (
    <span
      className={cn(
        'inline-flex items-center rounded-full border px-2.5 py-1 font-latin text-[11px] font-semibold',
        tones[tone],
        className,
      )}
    >
...
export function Eyebrow({
  ...
  return (
    <span
      className={cn(
        'inline-block text-[12px] font-bold tracking-[0.12em] rtl:tracking-normal',
        inverse ? 'text-journey-mint' : 'text-primary',
      )}
    >
      {children}
    </span>
...
export function Latin({ children }: { children: ReactNode }) {
  return (
    <span translate="no" className="bidi-iso font-latin">
      {children}
    </span>
  )
}
