...
export function Nav() {
  ...
  useEffect(() => {
    const onScroll = () => {
      const header = document.querySelector('.mobile-drawer-nav')
  ...
  return (
    ...
    >
      <nav
        aria-label={t('aria')}
        className="mobile-drawer-nav relative z-20 flex w-full max-w-[920px] items-center justify-between gap-3 rounded-full border border-border/80 bg-surface/90 px-2 py-2 text-navy backdrop-blur-md transition-shadow duration-200 shadow-header"
      >
        <a
          href="#top"
          translate="no"
          ...
          >
            ...
            >
              ...
              <div className="mt-7 grid gap-3">
                <Link href="/register" onClick={() => setDrawerOpen(false)} className="cta-solid min-h-[42px] px-4 py-2 text-[13px] font-bold">
                  {t('signup')}
                </Link>
                <Link
                  href="/login"
