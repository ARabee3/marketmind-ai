import { Controller, Get, HttpCode, HttpStatus, UseGuards, INestApplication } from "@nestjs/common";
...
class RbacTestController {
  ...
  businessRead() { return { status: "allowed", permission: PERMISSIONS.BUSINESS_READ }; }

  @Get("admin-library")
  @HttpCode(HttpStatus.OK)
  @Permissions(PERMISSIONS.ADMIN_MANAGE_LIBRARY)
  adminLibrary() { return { status: "allowed", permission: PERMISSIONS.ADMIN_MANAGE_LIBRARY }; }

  @Get("confirm-profile")
  @HttpCode(HttpStatus.OK)
  @Permissions(PERMISSIONS.DISCOVERY_CONFIRM_PROFILE)
  confirmProfile() { return { status: "allowed", permission: PERMISSIONS.DISCOVERY_CONFIRM_PROFILE }; }
}
...
describe("RBAC (e2e)", () => {
  ...
  describe("GET /api/v1/rbac/me/permissions", () => {
    ...
    it("should return all 8 permissions for admin (includes admin:publishing)", () =>
      request(app.getHttpServer())
        .get("/api/v1/rbac/me/permissions")
