import { ConflictException, INestApplication, ServiceUnavailableException, ValidationPipe } from "@nestjs/common";
...
import { DiscoveryStreamService } from "../src/modules/discovery/discovery-stream.service";
...
describe("Discovery public contract (e2e)", () => {
  ...
  const discoveryService = { startPreparedDiscovery: jest.fn(), getStatus: jest.fn() };
  ...
  beforeAll(async () => {
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        {
          provide: DiscoveryStreamService,
          useValue: { getStream: jest.fn() },
        },
      ],
    }).compile();
  ...
  it("accepts a valid intake and exposes the real Socket.IO namespace", () => {
    ...
    return request(app.getHttpServer())
      ...
      .send({ language_mode: "mixed", intake: { business_name: "Koshary Corner", business_type: "restaurant", city: "Cairo" } })
      .expect(202)
      .expect((response) => {
        ...
        expect(discoveryService.startPreparedDiscovery).toHaveBeenCalledWith(
          "owner-user-id",
          expect.objectContaining({ intake: expect.objectContaining({ business_name: "Koshary Corner" }) }),
        );
      });
  });
  ...
  it("validates session identifiers before calling the service", () =>
    request(app.getHttpServer())
      ...
      .set("Authorization", `Bearer ${ownerToken}`)
      .expect(400)
      .expect(() => expect(discoveryService.getStatus).not.toHaveBeenCalled()));
  ...
  it("returns a conflict for a conversation action in the wrong state", () => {
    conversationService.respondToDiscovery.mockRejectedValue(
      new ConflictException("Discovery session is not in a valid state for this action"),
    );
  ...
  it("returns 503 with DISCOVERY_ENQUEUE_FAILED when enqueue fails", () => {
    discoveryService.startPreparedDiscovery.mockRejectedValue(
      new ServiceUnavailableException({ code: "DISCOVERY_ENQUEUE_FAILED", message: "Redis connection refused" }),
    );
