...
describe("ContentProcessor - Static Asset Generation", () => {
  ...
  beforeEach(async () => {
    packRepo = {
      ...
      markAssetReady: jest.fn().mockResolvedValue({ changed: true }),
      markAssetFailed: jest.fn().mockResolvedValue({ changed: true }),
    } as any;
    ...
    assetStorage = {
      ...
      retrieve: jest.fn().mockResolvedValue(Buffer.from("asset-bytes")),
    } as any;
  ...
  describe("handleGenerateStaticAsset", () => {
    ...
    it("should store ready asset with checksum", async () => {
      ...
      expect(packRepo.markAssetReady).toHaveBeenCalledWith({
        assetId: "asset-1",
        contentItemVersionId: "version-1",
        mimeType: "image/png",
        width: 800,
    ...
    it("should handle failed asset generation", async () => {
      ...
      expect(packRepo.markAssetFailed).toHaveBeenCalledWith({
        assetId: "asset-1",
        contentItemVersionId: "version-1",
