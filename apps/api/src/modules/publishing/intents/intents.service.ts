...
export class IntentsService {
  ...
  private async runLocalAction(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // P2 (#119 review, non-blocking truthfulness): a MANUAL_EXPORT must NOT
      // record EXPORTED/SUCCEEDED until a checksum-addressed archive actually
      // exists. The archive bytes are generated by the #121 boundary, not
      // synthesized here. We therefore keep the intent in DISPATCHING (in
      // flight), record only the `manual_archive_pending` export metadata row,
      // and create NO result row — the UI shows a pending export, never a fake
      // downloadable artifact. #121 will later produce the archive + checksum
      // and complete the intent to EXPORTED/SUCCEEDED.
      if (mode === "MANUAL_EXPORT") {
        const attempt = await tx.publishingAttempt.create({
          data: {
            ...
            // In-flight until #121 finishes the archive — never SUCCEEDED here.
            status: "RUNNING",
            idempotencyKey: dto.idempotencyKey,
            workflowVersion: "local-v1",
            dispatchedAt: now,
            startedAt: now,
            // No finishedAt — the action is pending.
          },
        });
        ...
        // Intent stays DISPATCHING (already set above). No result, no SUCCEEDED.
        this.logger.log(
          `MANUAL_EXPORT dispatched for intent=${intentId} by user=${userId} → pending archive ${artifactId} (#121)`,
        );
        return {
          intent: this.projectIntent(
            await tx.publishingIntent.findUniqueOrThrow({
              where: { id: intentId },
            }),
          ),
          attempt,
          pendingArchiveArtifactId: artifactId,
          replayed: false,
        };
      }
