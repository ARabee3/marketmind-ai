...
const RETRYABLE_STATUSES: PublishingIntentStatus[] = [
  "FAILED",
  "ACTION_REQUIRED",
];
...
export class IntentsService {
    ...
    return this.prisma
      ...
      .then(async (updated) => {
        if (updated.scheduledUtcAt) {
          ...
          const jobKey = `publish:${intentId}:${updated.version}`;
          await this.dispatchQueue.add(
            "dispatch",
            {
              intentId,
  ...
  private async runLocalAction(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      const outcome = mode === "MANUAL_EXPORT" ? "EXPORTED" : "SIMULATED";

      // Export: record the archive metadata. The real archive artifact is BUILT by
      // the #121 boundary; this slice does NOT synthesize a fake file. We label
      // the row `manual_archive_pending` with a `pending:` destination so the UI
      // never presents a non-existent artifact as a real downloadable archive
      // (#119 non-blocking truthfulness gap). The checksum binds the intent +
      // candidate + artifact id deterministically so #121 can verify the build.
      if (mode === "MANUAL_EXPORT") {
        const archiveChecksum = crypto
          .createHash("sha256")
          .update(
            `${intentId}|${intent.candidateId}|${intent.candidate.candidateChecksum}|${artifactId}`,
          )
          .digest("hex");
        await tx.publishingExportMetadata.create({
          data: {
            ...
            destinationRef: `pending:#121:${businessId}/${artifactId}`,
      ...
      const result = await tx.publishingResult.create({
        data: {
          ...
          remoteUrl: null,
          exportArtifactId: mode === "MANUAL_EXPORT" ? artifactId : null,
          // Truthfulness (#119 non-blocking): a MANUAL_EXPORT is a LOCAL action
          // that has NOT been published by a provider. Label it so the UI never
          // presents it as real provider publication; the real archive artifact
          // is generated by the #121 boundary, not synthesized here.
          simulationLabel:
            mode === "SIMULATION" ? "SIMULATION" : "MANUAL_EXPORT_LOCAL",
          errorCode: null,
          retryable: false,
          occurredAt: now,
        },
