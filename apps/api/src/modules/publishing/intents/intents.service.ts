...
export class IntentsService {
  ...
  private async runLocalAction(
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // Export: record the generated archive metadata (checksum + destination).
      if (mode === "MANUAL_EXPORT") {
        ...
        await tx.publishingExportMetadata.create({
          data: {
            intentId,
            exportType: "manual_archive",
            destinationRef: `exports/${businessId}/${artifactId}.zip`,
      ...
      const result = await tx.publishingResult.create({
        data: {
          ...
          simulationLabel: mode === "SIMULATION" ? "SIMULATION" : null,
          errorCode: null,
          retryable: false,
          occurredAt: now,
        },
