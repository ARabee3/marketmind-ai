 ...
 * immutable checksum-addressed bytes from the same store the dispatch-time
 * integrity validator trusts.
 */
@Controller("internal/v1/publishing/media-fetch")
export class MediaFetchController {
  constructor(
    private readonly store: PublishingAssetStore,
  ...
  getAsset(
    ...
    if (!token || !attemptId || !exp) {
      throw new BadRequestException("PUBLISHING_ASSET_UNAVAILABLE: missing token");
    }
    const expMs = Number(exp);
    if (!Number.isFinite(expMs) || !this.tokens.verify({ token, attemptId, assetId, expMs })) {
      throw new GoneException(
        "PUBLISHING_ASSET_UNAVAILABLE: provider-fetch token invalid or expired",
      );
    }
    const record = this.store.getAsset(assetId);
    if (!record) {
