...
async function load(name: string): Promise<ContentPolicyFixture> {
  return JSON.parse(await readFile(new URL(name, examplesUrl), "utf8")) as ContentPolicyFixture;
}
...
type InvalidMutation =
  ...
  | { readonly kind: "claim"; readonly claim_type: ContentPolicyFixture["item_version"]["claim_sources"][number]["claim_type"]; readonly approved: boolean }
  | { readonly kind: "protected_text_mutated" }
  | { readonly kind: "asset_status"; readonly value: "missing" | "failed" | "blocked" }
  ...
  switch (testCase.mutation.kind) {
    ...
    case "approved_decision_without_asset":
      fixture.decision = {
        ...
        decision: "approved",
        revision_notes: null,
        decided_by_user_id: fixture.week_context.confirmed_by_user_id,
...
assert.deepEqual(valid.issues, [], `valid fixture failed: ${JSON.stringify(valid.issues)}`);
...
const week13 = validateContentSchema(
  JSON.parse(await readFile(new URL("content-week-13.invalid.json", examplesUrl), "utf8")),
);
assert(
...
const emptyAlt = validateContentSchema(
  JSON.parse(await readFile(new URL("content-empty-alt-text.invalid.json", examplesUrl), "utf8")),
);
assert(
...
const approvedCandidate = JSON.parse(
  await readFile(new URL("publication-candidate-approved.example.json", examplesUrl), "utf8"),
...
tamperedCandidate.caption = "Tampered caption that no longer matches the stored checksum.";
assert(
...
const revokedCandidate = structuredClone(approvedCandidate);
revokedCandidate.candidate_state = "revoked";
assert.equal(revokedCandidate.candidate_state, "revoked", "revoked candidate state");
