...
- **No OpenAPI/Swagger.** `@nestjs/swagger` is not a dependency and `SwaggerModule` is
  ...
  each module's `dto/`. Treat the examples below as illustrative and reconcile exact field
  names against those types. 
...
{
  "accessToken": "<jwt>",
  "user": { "id": "…", "email": "owner@example.com", "roles": ["owner"] }
}
```
...
```
> Field names inside `user` are illustrative — the authoritative shape is `SafeUser` in
> the auth module.
...
runs asynchronously on a queue — the client then watches `GET :sessionId/status` or the
SSE stream.
...
```json
{ …StartDiscoveryResponse (sessionId + initial state)… }
```
...
Four path-filtered GitHub Actions workflows run on PRs and pushes to `main`
(verified in `.github/workflows/`):

| Workflow | Jobs (verified) | Status |
|---|---|---|
| `api-ci.yml` | **build-and-unit**: contracts check → build API → Jest unit (`--runInBand`). **integration**: spins up Postgres 16 + Redis 7 services, generates Prisma client, `prisma migrate deploy`, runs E2E + DB integration tests. 
| `web-ci.yml` | **check** (typecheck + lint + vitest + dictionary parity), **build** (production `next build`), **e2e** (Playwright, chromium). |
| `ai-ci.yml` | **eval-smoke**, **content-eval** (deterministic suite + `--hard-guardrails-only` threshold), **agentic-phase4-5** safety smoke, **phase0-durability** (Postgres-backed LangGraph interrupt/resume gate). Uses `uv sync --frozen`. 
| `build-push-images.yml` | Matrix builds `api`/`web`/`ai` Docker images and pushes to `ghcr.io/<owner>/marketmind-ai-{api,web,ai}:latest` + `:<sha>`. 
