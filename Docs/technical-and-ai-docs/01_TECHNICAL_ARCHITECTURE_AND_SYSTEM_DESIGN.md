...
Weekly Content generation ─► Owner approval ─► Publish / Export
   │                                                      │
   ▼                                                      ▼
Facebook performance monitoring ─► AI optimization proposal ─► Owner approval
```
...
| `billing` | Bundles, wallet/points ledger, checkouts, provider webhooks | for `fake`/`paymob`; `geidea` not implemented |
...
`Docs/planning/07_GIT_CONVENTIONS.md`. Four path-filtered GitHub Actions workflows gate
`main` (API, web, AI, image build). Full CI/CD detail — including which gates are real
and which are aspirational — is in
...
|---|---|---|
| Auth + RBAC, discovery, strategy, content v1+v2, performance + optimization, publishing intents/targets/exports, health | implemented | On the live path. |
